...
with open(geometry_file_path, "w") as file:
    for i in range(100):
        ...
        target.location.x = random() * 2
        target.location.y = random() * 2
        target.location.z = random() * 2
        target.active_material.node_tree.nodes['Principled BSDF'].inputs["Base Color"].default_value = color
        f = "image_" + str(ts) + ".png"
